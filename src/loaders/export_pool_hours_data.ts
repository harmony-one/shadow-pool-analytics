...
import {getSwapEvents, getPositions, getPositionsBurns, getPoolHourDatas} from "../api";

export const loadData = async (poolSymbolBase: string) => {
  const poolSymbol = poolSymbolBase || 'USDC.e/USDT'
  let timestampFrom = moment().subtract(14, 'days').unix()

  let continueLoop = true
  let swaps: any[] = []
  do {
    const newSwaps = await getPoolHourDatas({
      first: 1000,
      filter: {
        poolSymbol,
        startOfHour_gt: timestampFrom
      },
      sort: {
    ...
    if(newSwaps.length > 0) {
      timestampFrom = Number(newSwaps[newSwaps.length - 1].startOfHour)
    }
    swaps.push(...newSwaps)
    console.log(`timestampFrom=${timestampFrom}, new=${newSwaps.length}, total=${swaps.length}`)
  } while (continueLoop)

  swaps.sort((a, b) => Number(a.startOfHour) - Number(b.startOfHour))
  ...
  const exportFileName = `export/pool_hours_data_${
